...
Generate a single tutorial chapter from a structured spec. Triggers on "generate chapter", "create the [chapter] tutorial", "write chapter [N]", "/chapter-gen". Takes a chapter spec (from curriculum-planner or inline) and produces a Markdown file following the configured voice guide, with MkDocs-compatible formatting.
...
```bash
brunnr install chapter-generator
```

**Tags:** `Educational Suite` 

---

## Security Scan

!!! warning "FLAGGED — 1 finding(s)"
    This skill has security findings that require review before installation.

??? warning "View findings"

    | Threat Class | Severity | Description | Line |
    |-------------|----------|-------------|------|
    | `steganographic` | **FLAG** | hidden command in HTML comment | 242 |
...
| Property | Value |
|----------|-------|
| **Path** | `skills/chapter-generator/SKILL.md` |
| **Size** | 9.8 KB |
| **SHA-256** | `abfff61496d5c4409392a528734c2fa38f18d638047223ef9d2fde2f7a60de74` |
